...
public class SchematicRender : MonoBehaviour
{
    ...
    public void RenderSchematic()
    {
        ...
        float drawSize = (lastDepth.RealToVirtualScale() / 2) + (ChristmasTreeGenerator.HeadSize / 2);

        _printCamera.orthographicSize = drawSize;
        _printCamera.transform.position = new Vector3(0, -drawSize + (ChristmasTreeGenerator.HeadSize), -10);
